import streamlit as st

from unseal.interface import utils
from unseal.interface.commons import SESSION_STATE_VARIABLES

def layer_change():
    utils.text_change()

# perform startup tasks
utils.startup(SESSION_STATE_VARIABLES, './registered_models.json')

with st.sidebar:
    utils.create_sidebar()
    
    sample = st.checkbox('Enable sampling', value=False, key='sample')
    if sample:
        utils.create_sample_sliders()
        utils.on_sampling_config_change()
    
    if "storage" not in st.session_state:
        st.session_state["storage"] = ""
        
    # select layer
    if st.session_state.num_layers is None:
        options = list()
    else:
        options = list(range(st.session_state.num_layers))
    st.selectbox('Layer', options=options, key='layer', on_change=layer_change, index=0)
    
    
    #TODO: text change doesn't work for single input
    # input
    placeholder = st.empty()
    placeholder.text_area(label='Input', on_change=utils.on_text_change, key='input_text', value=st.session_state.storage, kwargs=dict(storage_key="storage", text_key='input_text'))
    if sample:
        st.button(label="Sample", on_click=utils.sample_text, kwargs=dict(storage_key="storage", label='Input', key="input_text"), key="sample_text")

    # sometimes need to force a re-render
    st.button('Show Attention', on_click=utils.text_change)